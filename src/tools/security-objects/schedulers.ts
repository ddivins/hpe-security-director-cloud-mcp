...
export function registerSchedulerTools(server: McpServer, client: SdCloudClient) {
  server.registerTool(
    'sdcloud_list_schedulers',
    {
      description: 'List scheduler objects with optional pagination, filtering, and sorting.',
      inputSchema: listParams,
    },
    async (args) => {
      try {
        const data = await client.request('GET', '/api/v1/schedulers', { query: args });
  ...
  server.registerTool(
    ...
    async ({ body }) => {
      try {
        const data = await client.request('POST', '/api/v1/schedulers', { body });
        return toToolResult(data);
      } catch (e) {
        return toErrorResult(e);
      }
  ...
  server.registerTool(
    'sdcloud_get_scheduler',
    {
      description: 'Get a scheduler object by UUID.',
      inputSchema: { uuid: z.string() },
    },
    async ({ uuid }) => {
      try {
        const data = await client.request('GET', '/api/v1/schedulers/{uuid}', { pathParams: { uuid } });
        return toToolResult(data);
      } catch (e) {
        return toErrorResult(e);
      }
  ...
  server.registerTool(
    'sdcloud_update_scheduler',
    {
      description: 'Update a scheduler object by UUID.',
      inputSchema: { uuid: z.string(), body: jsonBody },
    },
    async ({ uuid, body }) => {
      try {
        const data = await client.request('PUT', '/api/v1/schedulers/{uuid}', {
          pathParams: { uuid },
          body,
        });
        return toToolResult(data);
  ...
  server.registerTool(
    'sdcloud_delete_scheduler',
    {
      description: 'Delete a scheduler object by UUID.',
      inputSchema: { uuid: z.string() },
    },
    async ({ uuid }) => {
      try {
        const data = await client.request('DELETE', '/api/v1/schedulers/{uuid}', {
          pathParams: { uuid },
        });
        return toToolResult(data);
      } catch (e) {
